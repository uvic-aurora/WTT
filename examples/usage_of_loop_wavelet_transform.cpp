...
int main() {
  ...
  // Compute 3-level Loop forward wavelet transform.
  bool res = wtlib::loop_analyze(mesh, coefs, 3);
  if (res) {
    // Compute 3-level Loop inverse wavelet transform.
    wtlib::loop_synthesize(mesh, coefs, 3);
  } else {
